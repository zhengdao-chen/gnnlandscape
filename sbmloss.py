...
avg_A = np.zeros([n * 2, n * 2])
avg_B = np.zeros([n * 2, n * 2])
for i in range(1000):
	A = get_adjacency_matrix(n, 0.8, 0.2)
	w, v = eigh(A)
	avg_A += A / 1000
	
for i in range(1000):
	B = get_adjacency_matrix_new(n, 0.8, 0.2)
	w, v = eigh(B)
	avg_B += B / 1000
...
X_tensor = np.zeros([len(n_lst), n_samples, d, d])
Y_tensor = np.zeros([len(n_lst), n_samples, d, d])
A_tensor = np.zeros([len(n_lst), n_samples, d, d])
B_tensor = np.zeros([len(n_lst), n_samples, d, d])

w_tensor = np.zeros([len(n_lst), n_samples, n_lst[len(n_lst) - 1] * 2])
...
y_2 = arr_2.reshape(1, 1200000)
y_3 = arr_3.reshape(1, 1200000)
y_4 = arr_4.reshape(1, 1200000)
